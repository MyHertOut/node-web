...
const Mysql = require('./dialects/mysql2');
...
class Db {
    ...
    createClient(database) {
        ...
        // using ./dialect/mysql default
        if (options.dialectModule !== 'sequelize' && options.dialect === 'mysql') {

            let db = new Mysql(options);
            db.authenticate()
                .then(() => {
                    //just auth testing
                ...
                .catch(err => {
                    console.error('Unable to connect to the database:', this.dbType, err);
                });

            cachedInstances[this.dbType][database] = db;
            return db;
    ...
    use(database) {
        ...
        if (this.dialectModule !== 'sequelize' && this.dbType === 'mysql') {
            return db;
        } else {
